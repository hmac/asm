...
_main:
  ...
  mov rdi, rsp
  mov rsi, rax
  call _sum_instructions
                        ; rax now holds a 32 bit number
                        ; The high 16 bits is the horizontal position.
                        ; The low 16 bits is the depth.
                        ; We want to multiply them together.

  mov rcx, 0            ; Put the high 8 bits in cx
  mov ecx, eax
  shr ecx, 16
  and eax, 0xffff       ; Clear the high 8 bits of eax

  mul ecx               ; Multiply eax and ecx
                        ; rax now holds the product of position and depth.
...
; u48 parse_instructions(u8* input, u64 len)
; rax                    rdi        rsi
...
; It returns a 32 bit number in the format specified in _parse_instruction.
_sum_instructions:
                        ; rdi: pointer to the input buffer
  mov rdx, rsi          ; rdx: pointer to the end of the input buffer
  add rdx, rdi
  mov rsi, 0            ; rsi: the accumulating sum
...
; void parse_instruction(u8* input, u32)
; rax                    rdi        rsi
; This function takes a pointer to an ASCII buffer containing a single submarine
; instruction, and an integer. Instructions have the grammar (forward|down|up) [0-9].
; It returns the given integer, after adding it to a 32 bit integer.
; The high 16 bits are the horizontal position.
; The low 16 bits are the depth.
; "forward" increases the horizontal position.
; "up" _decreases_ the depth.
; "down" _increases_ the depth.
; It advances the input pointer so that it points to the next instruction, if
; there is one.
_parse_instruction:
  mov rcx, 0
  mov r8b, [rdi]
  ...
  mov cl, [rdi+3]       ; Set the value byte to the ASCII digit
  sub cl, 48            ; Convert the value byte from ASCII to an integer
  sub rsi, rcx          ; Subtract this depth from the total
  add rdi, 5            ; Advance the input buffer to consume this instruction
  ret
_parse_instruction_down:
  mov cl, [rdi+5]       ; Set the value byte to the ASCII digit
  sub cl, 48            ; Convert the value byte from ASCII to an integer
  add rsi, rcx          ; Add this depth to the total
  add rdi, 7            ; Advance the input buffer
  ret
_parse_instruction_forward:
  mov cl, [rdi+8]       ; Set the value byte to the ASCII digit
  sub cl, 48            ; Convert the value byte from ASCII to an integer
  shl ecx, 16           ; Shift the value to the high 16 bits
  add rsi, rcx          ; Add this position to the total
